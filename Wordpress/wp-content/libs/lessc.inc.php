...
class lessc
{
  ...
  function chunk()
  {
    ...
    if ($this->literal('}'))
    {
      ...
      if (!empty($tags)) $out = $this->compileBlock($tags, $env);
    ...
    if ($this->tags($tags, true, '>') && ($this->argumentValues($argv) || true) && $this->end())
    {
      ...
      if (!empty($env['__args']))
      {
        foreach ($env['__args'] as $arg)
        {
          ...
          if ($value == null && isset($arg[1])) $value = $arg[1];
  ...
  function expHelper($lhs, $minP, $needWhite = true)
  {
    ...
    while ($this->match(self::$operatorString . ($needWhite ? '\s+' : ''), $m) && self::$precedence[$m[1]] >= $minP)
    {
      ...
      else $lhs = $this->evaluate($m[1], $lhs, $rhs);
  ...
  function value(&$value)
  {
    ...
    {
      $tmp = $this->getEnv($a[0]);
      if ($tmp && isset($tmp[$a[1]])) $value = end($tmp[$a[1]]);
      return true;
    }
  ...
  function quoted($s)
  {
    if (preg_match('/^("|\').*?\1$/', $s, $m)) return $m[1];
    else return false;
  }
  ...
  public static function ccompile($in, $out, $compress = true)
  {
    if (!is_file($out) || filemtime($in) > filemtime($out))
    {
      $less = new lessc($in);
      $css = $less->parse();
      file_put_contents($out, ($compress) ? self::compress($css) : $css);
      return true;
    }
